				...
				<form class="form-horizontal" method="post" action="{{url('selectiveprocess')}}">
					...
					<fieldset>
						...
						<div class="form-group">
							...
							<div class="col-md-7">
								<div class="input-group">
									...
									<input  name="data_inicio" required="required" type="date" placeholder="Data de Inicio" class="form-control input-md" value="{{$sp->data_inicio}}">
								</div>
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-md-7">
								<div class="input-group">
									...
									<input id="data_fim" name="data_fim" required="required" type="date" placeholder="Data do Fim" class="form-control input-md" value="{{$sp->data_fim}}">
								</div>
							</div>
						</div>
						...
						<div class="form-group">
							...
							@if(($course->pivot->course_id) == ($course->id))	
								<div class="row">
								<div class="col-md-2 cursos"></div>
									<div class="col-md-5 cursos">											
										<label class="control-label" for="curso_{{ $course->id }}">{{$course->nome}}</label>
										<input type="checkbox" id="curso_{{$course->id}}" name="curso_id[{{ $course->id }}][id]" value="{{ $course->id }}" checked="checked">
									</div>	
									<div class="col-md-2">
										<label class="control-label">Vagas</label>
									</div>
									<div class="col-md-2">
										<input value="{{$course->pivot->vagas}}" id="nome" name="curso_id[{{ $course->id }}][vagas]" type="text" class="form-control" >
									</div>
								</div>
							@else
							<div class="row">
								<div class="col-md-2 cursos"></div>
									<div class="col-md-5 cursos">											
										<label class="control-label" for="curso_{{ $course->id }}">{{$course->nome}}</label>
										<input type="checkbox" id="curso_{{$course->id}}" name="curso_id[{{ $course->id }}][id]" >
									</div>	
									<div class="col-md-2">
										<label class="control-label">Vagas</label>
									</div>
